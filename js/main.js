...
/* скроллинг хедера ( фиксация вверху и появление при скроллинге )*/
$(function() {
	var scrolled;
...
$(function() { 
	...
	$(document).mouseup(function (e){ // событие клика по веб-документу
		if (!$('.navigation-hidden ul').is(e.target) // если клик был не по нашему блоку
		    && $('.navigation-hidden ul').has(e.target).length === 0) { // и не по его дочерним элементам		
				if($(window).innerWidth() > 991){
					$('.navigation-hidden').animate({
						top: '-100%'
					},500) // скрываем его
